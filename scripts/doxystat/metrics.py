...
from compound import DoxCompoundKind, DoxMemberKind, DoxSectionKind, MixedContainer
...
class Metrics:
    ...
    def print(self):
        ...
        with open('stats.txt', 'w', encoding='utf-8') as file:
            file.write("Undocumented classes\n")
            file.writelines(self.listClass)
...
def parse_members(compounddef,sectiondef,metrics):
    ...
    for memberdef in sectiondef.get_memberdef():
        #print(f"        {memberdef.get_name()} comments={is_documented(memberdef)}")
...
def parse_index(inDirName):
    metrics = Metrics()
    rootObj = index.parse(inDirName+"/index.xml",True)
